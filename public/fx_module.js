    ...
    ApplyAccretionDisk: function(mesh, colorStr) {
        ...
        const fragmentShader = `
            ...
            void main() {
                ...
                // Rotación dinámica basada en tiempo
                float dist = length(vUv - 0.5);
                float angle = atan(vUv.y - 0.5, vUv.x - 0.5);
                angle += uTime * 3.0 * (1.0 / (dist + 0.1)); // Rotación diferencial
                
                vec2 polarUV = vec2(cos(angle)*dist, sin(angle)*dist) * 20.0;
                float turbulence = fbm(polarUV - vec2(uTime * 0.5));
                
                // Atenuación en los bordes para simular densidad del gas
                float edgeFade = sin(vUv.y * 3.14159) * sin(vUv.x * 3.14159);
